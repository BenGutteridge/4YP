...
from utils_update_last_week import sample_Z, sample_pi, sample_mu, sample_lambda, samples
from utils_update_last_week import q_Z, q_pi, q_mu, q_lambda

# Evaluations of p() distributions
from utils_update_last_week import p_Z_given_pi, p_pi, p_mu_given_lambda, p_lambda
from utils_update_last_week import p_X_given_Z_mu_lambda

# Misc
from utils_update_last_week import responsibility, draw_ellipse, plot_GMM, cols
...
x = X[n]
Z, pi, mu, lam, r_nk = samples(alpha**2, beta, [np.dot(W[k].T, W[k]) for k in range(K)], m, nu, x, K)
...
def calculate_ELBO(x,Z,pi,mu,lam,alpha,beta,W,m,nu,K,N):
    alpha = alpha**2
    W = [np.dot(W[k].T, W[k]) for k in range(K)]
